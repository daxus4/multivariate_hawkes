...
class EveryTimePredictionTesterMultivariate(TimePredictionTester):
    ...
    def get_multivariate_predictions(self) -> pd.DataFrame:
        ...
        for simulation_df in self._simulation_event_dfs:
            ...
            prediction_df["Predicted MP UP BI POS Time"].append(predictions["MP_UP_BI_POS"][0])
            prediction_df["Predicted MP DOWN BI POS Time"].append(
                predictions["MP_DOWN_BI_POS"][0]
            )
            prediction_df["Predicted MP UP BI NEG Time"].append(predictions["MP_UP_BI_NEG"][0])
            prediction_df["Predicted MP DOWN BI NEG Time"].append(
                predictions["MP_DOWN_BI_NEG"][0]
            )
