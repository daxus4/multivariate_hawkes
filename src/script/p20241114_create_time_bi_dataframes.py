...
def save_training_df_if_not_exist(
    ...
    coe_training_dataframe_path = os.path.join(
        coe_training_dataframe_folder,
        coe_training_dataframe_filename
    )
...
if __name__ == "__main__":
    ...
    events_conf_map = get_conf(
        os.path.join(
            ...
            'mid_price_change_events_conf.yml'
        )
    )
    ...
    for pair in coe_conf.pairs:
        ...
        for loading_info, bi_level, training_time_seconds, simulation_period_seconds in product(
            ...
            for start_simulation_time in loading_info.start_times:
                ...
                lob_period = lob_period_extractor.get_lob_period(start_coe_training_time, end_simulation_time)
                lob_df_for_events = lob_period.get_lob_df_with_timestamp_column()

                lob_df_for_events['Timestamp'] = lob_df_for_events['Timestamp'] - start_coe_training_time
                ...
                lob_df_for_events_simulation['Timestamp'] = lob_df_for_events_simulation['Timestamp'] - start_simulation_time

                simulation_methods_pair_params_dirs = get_simulation_methods_pair_params_dirs(pair)

                for simulation_method_pair_params_dir in simulation_methods_pair_params_dirs:
                    simulation_filename = f'{prefix_lob}_{start_simulation_time}.tsv'

                    simulation_df = pd.read_csv(
                        os.path.join(simulation_method_pair_params_dir, simulation_filename),
                        sep='\t'
                    )
                    ...
                    simulation_df_for_coe = pd.concat(
                        [
                            ...
                            lob_df_for_events_simulation[['BaseImbalance', 'Return']].reset_index(drop=True)
                        ], 
                        axis=1
                    )

                    decomposed_path = get_decomposed_path(simulation_method_pair_params_dir)
                    ...
                    coe_simulation_dataframe_path = os.path.join(
                        coe_simulation_dataframe_folder,
                        simulation_filename
                    )

                    simulation_df_for_coe.to_csv(
                        coe_simulation_dataframe_path,
                        sep='\t',
                        index=False
                    )
