...
def extract_lob_events(df: pd.DataFrame) -> Dict[str, np.ndarray]:
    """
    ...
    # Event 1: Mid price increases
    mask_event_1 = ((df["AskPrice1"] + df["BidPrice1"])/2) > ((df["AskPrice1_prev"] + df["BidPrice1_prev"])/2)
    timestamps_event_1 = df[mask_event_1]["Timestamp"].to_numpy()

    # Event 2: Mid price decrease
    mask_event_2 = ((df["AskPrice1"] + df["BidPrice1"])/2) < ((df["AskPrice1_prev"] + df["BidPrice1_prev"])/2)
    timestamps_event_2 = df[mask_event_2]["Timestamp"].to_numpy()

    # Event 3: AskSize1 increases OR BidSize1 decreases (but not to zero)
    mask_event_3 = (
        (df["AskSize1"] > df["AskSize1_prev"])
        & (df["AskPrice1"] == df["AskPrice1_prev"])
    ) | (
        (df["BidSize1"] < df["BidSize1_prev"])
        & (df["BidPrice1"] == df["BidPrice1_prev"])
    )
    timestamps_event_3 = df[mask_event_3]["Timestamp"].to_numpy()

    # Event 4: AskSize1 decreases OR BidSize1 increases
    mask_event_4 = (
        (df["AskSize1"] < df["AskSize1_prev"])
        & (df["AskPrice1"] == df["AskPrice1_prev"])
    ) | (
    ...
    return {
        "MID_PRICE_UP": timestamps_event_1,
        "MID_PRICE_DOWN": timestamps_event_2,
        "BUY_PRESSURE_UP": timestamps_event_4,
        "SELL_PRESSURE_UP": timestamps_event_3,
    }
    ...
    for loading_info in loading_info_for_all_dfs:
        ...
        for start_simulation_time in loading_info.start_times:
            for training_time_seconds in training_conf.seconds_in_a_period:
                ...
                training_times_cpu.append(end_time_cpu - start_time_cpu)

                print(hawkes_kernel.score())

                params_dir = os.path.join(
                    CONST.TRAINED_PARAMS_FOLDER,
                    CONST.MULTIVARIATE_HAWKES + "_bid_ask",
                    training_conf.pair,
