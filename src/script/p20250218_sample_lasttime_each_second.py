...
def get_df_last_event_each_second(df: pd.DataFrame, col: str = "real") -> pd.DataFrame:
    """
    Get only the last event in each second.
    """
    df = df.iloc[1:]

    return df.groupby(df["real"].floordiv(1)).tail(1).reset_index(drop=True)
...
def filter_and_save_orderbook_changes(
    ...
    for file_name in os.listdir(abs_dir):
        if file_name.startswith("orderbook_changes_") and file_name.endswith(".tsv"):
            ...
            filtered_df = get_df_last_event_each_second(df)
            filtered_df = get_df_without_increasing_predictions(filtered_df)
